...
page_id = "bbcnews" # page id of any public page
...
def getFacebookPageData(page_id, access_token, number_of_statuses):
...
def getReactionsForPagePosts(status_id, access_token):
...
def processFacebookPageFeedStatus(status, access_token):
    ...
    reactions = getReactionsForPagePosts(status_id, access_token) if status_published > '2016-02-24 00:00:00' else {}
...
def scrapeFacebookPageStatus(page_id, access_token):
    with open('%s_facebook_page_statuses.csv' % page_id, 'w') as file:
        w = csv.writer(file)
